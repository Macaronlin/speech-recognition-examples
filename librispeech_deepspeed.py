...
# ============================================= COLLATE FUNCTION =======================================================
def collate(data):
    spectrograms = [audio_transforms(waveform).squeeze(0).permute(1, 0).to(torch.float16) for (waveform, _, utterance, _, _, _) in data]
...
def fit(model, epochs, train_data_loader, valid_data_loader):
    ...
    for i in range(1, epochs + 1):
        ...
        for spectrograms, labels, input_lengths, label_lengths in tqdm(train_data_loader,
            ...
            loss = loss_func(model(spectrograms).log_softmax(2).permute(1, 0, 2).float(), labels, input_lengths, label_lengths)
            model.backward(loss)
            model.step()
